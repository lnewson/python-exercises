...
class MyCircle:
    ...
    # Formula = d(t) = |A(t) - B(t)| - (Ra + Rb)
    def surfaceDistance(self, other, dtime):
        """
        Determines the distance between circle surfaces
        """
        radiiAB = self.size + other.size

        # Calculate the position of each for the elapsed time
        posA = self.position + self.velocity * dtime + 0.5 * (self.accel * (dtime ** 2))
        posB = other.position + other.velocity * dtime + 0.5 * (other.accel * (dtime ** 2))

        # Get the absolute value of posA - posB
        absPosAB = abs(posA - posB)

        return absPosAB - radiiAB

    def collide(self, other, dtime):
        """
        Checks for a collision and adjusts the vectors of the circles in the event of a collision
        """
        # Check for a collision. If the surface distance is less than or equal to 0 then we have had a collision.
        if self.surfaceDistance(other, dtime) <= 0:
            collisionVector = self.position - other.position
            collisionVector.normalize()
            self.velocity = self.velocity.reflect(collisionVector)
            other.velocity = other.velocity.reflect(collisionVector)
...
def startGame(screen):
    ...
    while True:
        ...
        for i, circle in enumerate(circles):
            circle.move(dtime)
            for otherCircle in circles[i + 1:]:
                circle.collide(otherCircle, dtime)
